...
var cat_str = new Object(); // zhangyh_100322
...
function init_rec_data()
{
	...
	//cat_str[0] = document.getElementById("show_cat_area") == null ? '' : document.getElementById("show_cat_area").innerHTML;  /* zhangyh_100322*/
}


function get_cat_recommend(rec_type, cat_id,topcat_id)  // zhangyh_100322
{
    if (rec_type == 1)
    {
	...
	else if (rec_type == 4)
    {
        ...
        if (typeof(cat_str[cat_id]) == "string")
        {
			document.getElementById("show_cat_area"+topcat_id).innerHTML = cat_str[cat_id];
            return;
        }
    }
	/* zhangyh_100322 end */
  ...
  Ajax.call('index.php?act=cat_rec', 'rec_type=' + rec_type + '&cid=' + cat_id+'&topcat_id='+topcat_id, cat_rec_response, "POST", "TEXT"); // zhangyh_100322
}
...
function cat_rec_response(result)
{
	...
	else if (res.type == 4)
    {
		...
		var ele = document.getElementById("show_cat_area"+res.topcat_id);
        cat_str[res.cat_id] = res.content;
    }
	/* zhangyh_100322 end */
