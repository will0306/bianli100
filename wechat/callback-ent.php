    ...
    public function responseMsg($db,$user,$base_url)
    {
		...
		if (!empty($postStr) or $debug == 1){
                ...
                if(empty($fromUsername)){
                    ...
                    {
                        $fromUsername = 'oyKzfsmRtNW5GQkCpU94TA3XV8aE';
                    }
                ...
                //初始化
//                $this->curl_get_contents();//todo 记录上线的程序
                //会员绑定进度
...
//                    echo "SELECT `user_id` FROM `$thistable` WHERE `user_name` ='$uname'";
//                    print_r($ret);exit;
                }
...
( '$fromUsername', '$keyword', $time);");
                ...
                if(empty($auto_reg['autoreg_rand']))
                {
                    $auto_reg['autoreg_rand'] = 6;
                }
                $ec_rand = substr(uniqid(rand()), 0, $auto_reg['autoreg_rand']);
                $ec_pwd_rand = $ec_pwd = $ec_pwd.$ec_rand;
               
                $ec_pwd = md5($ec_pwd);

                //by djks 140317  兼容老程序
...
//                $ret_223 = $db->getRow("SELECT * FROM `$thistable` WHERE `user_name` = '$fromUsername'");
//                if(strlen($ret_223['user_name']) == 28)
//                {
//                    $sql_del = "UPDATE `$thistable` SET `user_rank` = '99',`wxch_bd`='no',`wxid`='$fromUsername' WHERE `wxid` ='$fromUsername'";
//                    $db->query($sql_del);
//                }
            ...
            if($auto_reg['state'] == 1)
            {
                        ...
                        if(empty($wxch_user_name))
                        {
                            ...
                            if(empty($wxch_nobd_wxid))
                            {
                                ...
                                $ecs_user_id = $db->insert_id();
                                 //encrypt
                                $en_password = $this->encrypt($ec_pwd_rand,$pwd_key);
                                $db->query("UPDATE `wxch_user` SET `password` = '$en_password' WHERE `wxid` ='$fromUsername';");
                ...
                //老的绑定方式，是以user_name 28 位来判断的
                /*
                else
                {
                    $ret_22 = $db->getRow("SELECT * FROM `$thistable` WHERE `wxid` = '$fromUsername'");
//                    print_r($ret_22);exit;
                    if(!empty($ret_22['wxch_bd'])){
                        if(strlen($ret_22['wxch_bd']) == 'ok'){
                            $sql_del = "UPDATE `$thistable` SET `wxid`='' WHERE `wxid` = '$fromUsername';";
                        }
                        elseif(strlen($ret_22['wxid']) == 28)
                        {
                            $sql_del = "UPDATE `$thistable` SET `wxid`='' WHERE `wxid` = '$fromUsername';";
                        }
                        $db->query($sql_del);
                    }
                    $sql_update = "UPDATE  `$thistable` SET  `wxid` = '$fromUsername' WHERE  `user_name` = '$uname' ;";
                    $db->query($sql_update);
                }
                 * 
                 */
           ...
           //最早的消息
//           $contentStr = $postObj->Event.$keyword.'-debug';
//            $msgType = 'text';
//            $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
//            echo $resultStr;
//            exit;
           ...
           if($postObj->Event == 'subscribe')
           {
               ...
               else
               {
                   ...
                   //临时二维码
//                   $contents = 'scene_arr0:' . $scene_id_arr[0] . '+scene_arr1:' . $scene_id_arr[1] . '+qrscene:' . $qrscene . '-t';
//                   $this->mydebug($textTpl, $fromUsername, $toUsername, $time, $contents);
                   $scene_arr = explode('666',$scene_id_arr[1]);
                   if($scene_arr[1]>1)
                   {
                       if($scene_arr[0] == '11')
                       {
                       ...
                       {
                           $qr_function = 'subscribe';
                       }

                       $postObj->EventKey = 'affiliate_'.$qr_function.'_'.$scene_arr[1];
                   }
                   else
                   {
           ...
           elseif($postObj->Event == 'SCAN')
           {
               ...
               $scene_arr = explode('666',$qrscene);
//               $contents = 'scene_arr0:'.$scene_arr[0].'+scene_arr1:'.$scene_arr[1].'+qrscene:'.$qrscene.'-t';
//               $this->mydebug($textTpl, $fromUsername, $toUsername, $time, $contents);
               if($scene_arr[1]>1)
               {
                   if($scene_arr[0] == 11)
                   {
                       $qr_function = 'tempscan';
                   }
                   else
                   {
                       $qr_function = 'subscribe';
...
//                        $keyword = mb_convert_encoding($keyword, 'GBK','utf-8');
            ...
            if($wxch_bd == 'web')
            {
                ...
                if($keyword == 'ko' or $keyword == 'KO' or $keyword == 'Ko' or $keyword == '退出')
                {
//                    $contents = 'keyword:'.$keyword.'---setp:'.$setp;
//                    $this->mydebug($textTpl, $fromUsername, $toUsername, $time, $contents);
                    $db->query("UPDATE `wxch_user` SET `setp`= 0,`uname` = '' WHERE `wxid`= '$fromUsername';");
                    $contentStr = "您已退出会员绑定流程，再次绑定输入bd进入绑定流程";
                    $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
...
//                $keyword = mb_convert_encoding($keyword, 'GBK','utf-8');
                ...
                if(!$verifyLogin)
                {
//                    $contents = 'uname:'.$uname.'---pwd:'.$password;
//                    $this->mydebug($textTpl, $fromUsername, $toUsername, $time, $contents);
                ...
                else
                {
                    ...
                    $db->query("UPDATE `$users_table` SET `wxch_bd`='no',`wxid`='' WHERE `user_name`= '$uname';");    //todo member bd 
                    $db->query("UPDATE `wxch_user` SET `uname`='' WHERE `uname`= '$uname';");    //todo member bd                     
                }
                $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
...
( '$fromUsername', '$menu_message', $time);");
            ...
            {
                $pattern = '/\{\$([\w\d]+)\}/';
                $user_name = $ecs_user_name;
                $password = $ec_pwd_rand;
                if (preg_match($pattern, $auto_reg['autoreg_content']))
                {
                    $tpl_ret = preg_replace($pattern, '\$$1', $auto_reg['autoreg_content']);
                }

                $tpl_assign = '$auto_msg = "'.$tpl_ret.'";';
            ...
            //mydebug
//            $contents = 'uname:'.$uname.'--setp:'.$setp.'--keyword:'.$keyword;
//            $this->mydebug($textTpl, $fromUsername, $toUsername, $time, $contents);
            ...
            if($keyword == 'subscribe')
            {
                ...
                {
                    $pattern = '/\{\$([\w\d]+)\}/';
                    $tb_users = $db->prefix.'users';
                    $user_name = $db->getOne("SELECT `user_name` FROM `$tb_users` WHERE `wxid` = '$fromUsername'");
                    $de_password = $db->getOne("SELECT `password` FROM `wxch_user` WHERE wxid = '$fromUsername'");
                    $password = $this->decrypt($de_password,$pwd_key);
                    if (preg_match($pattern, $auto_reg['autoreg_content']))
                    {
                        $tpl_ret = preg_replace($pattern, '\$$1', $auto_reg['autoreg_content']);
                    }
            ...
            $this->getauto($db, $keyword, $textTpl,$newsTpl, $base_url, $m_url, $fromUsername, $toUsername, $time, $article_url,$auto_msg);
             
           
           // is me
           /*
             if($fromUsername == 'oMPu5uCyZM-TQ7OfxajZvgAM2Ea0')
             {
//                 //menu debug
                 if(!empty($postObj->EventKey)){
                 $msgType = "text";
                 $contentStr = 'msgType:'.$postObj->MsgType.$postObj->EventKey;
                 $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
                 echo $resultStr;
                 exit;
                 }
                 ...
                 $msgType = "text";
                 $contentStr = 'Event:'.$postObj->Event.'MsgType:'.$postObj->MsgType.'EventKey:'.$postObj->EventKey.'keyword:'.$keyword;
                 $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
                 echo $resultStr;
                 exit;
             }
...
//            $this->mydebug($textTpl, $fromUsername, $toUsername, $time, $contents);
            if($aff_arr[0] == 'affiliate')
            {
                ...
                if(!empty($aff_arr[2]))
                {
                   //$aff_arr[2] 推荐人user_id
                   $db_users = $db->prefix.'users';
                   $aff_query = "SELECT * FROM `$db_users` WHERE `user_id` = $aff_arr[2]";
                   ...
                   if(!empty($aff_db))
                   {
                       if($cfg_tuijian == 'true')
                       {
                           ...
                           $tb_shop_config = $db->prefix.'shop_config';
                           $affiliate_config = unserialize($db->getOne("SELECT `value` FROM ".$tb_shop_config. " WHERE code = 'affiliate';"));
                           // 0 1
                           if($affiliate_config['config']['separate_by'] == 0)
                           {
                               $aff_users = "UPDATE `$db_users` SET `parent_id` = '$aff_arr[2]' WHERE `wxid` = '$fromUsername'";
                               $db->query($aff_users);
                           }
...
//                        $goods_url = $oauth_location.$m_url.'goods.php?id='.$v['goods_id'];
			...
			elseif($keyword == 'member'||$keyword == 'Member'||$keyword == '会员中心')
            {
				$msgType = "text";
			...
			elseif($keyword == 'Sy'||$keyword == 'sy'||$keyword == '首页')
            {
				$msgType = "text";
				$contentStr = "<a href='".$m_url."index.php?wxid=".$fromUsername."'>点击进入商城首页</a>";
				$resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
				echo $resultStr;
                $this->insert_wmessage($db, $fromUsername, $w_message, $time,$belong);
                $this->plusPoint($db, $uname, $keyword, $fromUsername);
...
//            {
//                $msgType = "text";
//                $contentStr = '超链接：<a href="http://www.weixincaihong.com">微信彩虹</a>';
//                $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
//                echo $resultStr;
//                exit;
//            }
            ...
            elseif($keyword == 'dzp')
            {
                $get_baseurl = $this->get_baseurl($db);
                $data = $this->dzp($db,$get_baseurl,$fromUsername);
                $msgType = "news";
            ...
            {
                $get_baseurl = $this->get_baseurl($db);
                $data = $this->egg($db,$get_baseurl,$fromUsername,$m_url);
                $msgType = "news";
                $resultStr = sprintf($newsTpl, $fromUsername, $toUsername, $time, $msgType, $data['ArticleCount'],$data['items']);
                $w_message = '图文消息';
...
nation=大雁塔&mode=driving&region=西安';
            ...
            elseif($keyword == 'gzyhj')
            {
                ...
                $contentStr = $this->coupon($db,$fromUsername).$auto_msg;
                $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
                $this->insert_wmessage($db, $fromUsername, $contentStr, $time, $belong);
                $this->universal($fromUsername,$base_url,$keyword);
                echo $resultStr;
            ...
            elseif($keyword == 'subscribe')
            {
                ...
                $contentStr = $lang['regmsg'].$auto_msg;
                $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
            ...
            if(!empty($keyword))
            {
                ...
                $thistable = $db->prefix.'goods';
                //$goods_name = mb_convert_encoding($keyword, 'GBK','utf-8');
                $goods_name = $keyword;
...
('$fromUsername', '$keyword' , 1, $lasttime, $dateline);";
            ...
            else
            {
                if($keyword == 'qiandao')
                {
                    ...
                    //时间差计算
                    $time = time();
//                echo $time-(60*60*24);exit;
//                echo date("Y-m-d H:i:s",$time);
//                echo date("Y-m-d H:i:s",$thistime);
//                exit;
...
//                echo date("Y-m-d H:i:s",$db_lasttime);
//                echo $time-$db_lasttime.'---'.(60*60*24).'<hr>';exit;
//                echo 3600*24;
//                exit;
                    //重新更新
            ...
            foreach($wxch_points as $k=>$v){
                if($v['point_name'] == $keyword){
                    if($v['autoload'] == 'yes'){
                        ...
                        if($fromUsername == 'oIM-ajhetL3OwUfIm2DNgC1pW9Uk'){
                            echo $sql;
                        }

                        $db->query($sql);
                        $this->log_account_change($uname,$fromUsername,$points,$keyword);
                    }
                }
            }
     ...
     * @return  void
     */
        protected function log_account_change($uname, $fromUsername, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
        {
            if(!empty($uname))
            {
                $user_id = $GLOBALS['db']->getOne("SELECT `user_id` FROM ".$GLOBALS['ecs']->table('users')." WHERE `user_name` ='$uname'");
            }
            else
            {
                $user_id = $GLOBALS['db']->getOne("SELECT `user_id` FROM ".$GLOBALS['ecs']->table('users')." WHERE `wxid` ='$fromUsername'");
            }
            /* 插入帐户变动记录 */
            $account_log = array(
                'user_id'       => $user_id,
...
('$fromUsername', '$w_message', '$belong', '$time');";
        ...
        protected function dzp($db,$base_url,$fromUsername)
        {
            ...
            $dzp_title = $db->getOne("SELECT `lang_value` FROM `wxch_lang` WHERE `lang_name` = 'dzp_title'");
            $dzp_title_close = $db->getOne("SELECT `lang_value` FROM `wxch_lang` WHERE `lang_name` = 'dzp_title_close'");
            if(empty($dzp_title))
            {
                $dzp_title = '大转盘';
            }

            if(empty($dzp_title_close))
            {
                $dzp_title_close = '大转盘活动未开放';
            }

            if($prize_count <= 0)
            {
                $items = '<item>
                 <Title><![CDATA['.$dzp_title_close.']]></Title>
                 <PicUrl><![CDATA[]]></PicUrl>
                 <Url><![CDATA[]]></Url>
                 </item>';
            }
        ...
        //砸金蛋
        protected function egg($db,$base_url,$fromUsername,$m_url = '')
        {
            ...
            $zjd_title = $db->getOne("SELECT `lang_value` FROM `wxch_lang` WHERE `lang_name` = 'egg_title'");
            $zjd_title_close = $db->getOne("SELECT `lang_value` FROM `wxch_lang` WHERE `lang_name` = 'egg_title_close'");
            if(empty($zjd_title))
            {
                $zjd_title = '砸金蛋';
            }

            if(empty($zjd_title_close))
            {
                $zjd_title_close = '砸金蛋活动未开放';
            }

            if($prize_count <= 0)
            {
                $items = '<item>
             <Title><![CDATA['.$zjd_title_close.']]></Title>
             <PicUrl><![CDATA[]]></PicUrl>
             <Url><![CDATA[]]></Url>
             </item>';
            }
            else
            {
                ...
                if($oauth_state == 'true')
                {
                    ...
                    $gourl = $oauth_location.$base_url.'wechat/egg/index.php?pid='.$prize['pid'];
                }
                else
                {
        ...
        //自动回复
        protected function getauto($db,$keyword,$textTpl, $newsTpl, $base_url, $m_url, $fromUsername, $toUsername, $time, $article_url,$auto_msg='')
        {
                        ...
                        elseif($v['type'] == 2)
                        {
                            ...
                            foreach($res_ks as $kk=>$vv){
                             ...
                             <Url><![CDATA[".$gourl."]]></Url>
                                   ...
                                   {
                                       $items .= "<item>
                             <Title><![CDATA[".$auto_msg."]]></Title>
                             <Description><![CDATA[".$auto_msg."]]></Description>
                             </item>";
...
//            $menu_arr = json_decode($menu,TRUE);
        ...
        public function kuaidi($invoice_no,$shipping_name)
        {
                switch ($shipping_name) {
                    case '中国邮政':$logi_type = 'ems';break;
                    case '申通快递':$logi_type = 'shentong';break;
                    case '圆通速递':$logi_type = 'yuantong';break;
                    case '顺丰速运':$logi_type = 'shunfeng';break;
                    case '韵达快递':$logi_type = 'yunda';break;
                    case '天天快递':$logi_type = 'tiantian';break;
                    case '中通速递':$logi_type = 'zhongtong';break;
                    case '增益速递':$logi_type = 'zengyisudi';break;
        ...
        //通用快递查询
        public function tykuaidi($invoice_no,$shipping_code)
        {
        ...
        //最后通用函数，可以统一调用
        public function universal($wxid,$base_url,$keyword='')
        {
                
                $arr = explode("/",$base_url);
                $temp_arr_do = explode('.',$base_url);
                $temp_do = array('http://m','http://mobile');


                if(in_array($temp_arr_do,$temp_do))
                {
                    $gourl = 'http://www.'.$temp_arr_do[1].'.'.$temp_arr_do[2];
...
//		sort($tmpArr);
    ...
    private function get_baseurl($db)
    {
